...
//        }
     ...
     */
    @Override
    public void addReviewToShoe(Review review, Long shoeId) {
        Shoe shoe = shoeRepository.getOne(shoeId);
        Review shoeReview = new Review();
        shoeReview.setAuthor(review.getAuthor());
        shoeReview.setMessage(review.getMessage());
        shoeReview.setDate(LocalDate.now());
        shoe.getReviews().add(shoeReview);

        reviewRepository.save(shoeReview);
    }
}