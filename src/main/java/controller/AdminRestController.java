...
public class AdminRestController {
     ...
     */
    @Autowired
    public AdminRestController(UserService userService, ShoeService shoeService, OrderService orderService) {
    ...
    public ResponseEntity<?> updateShoe(
        ...
        if (bindingResult.hasErrors()) {
            ...
            ShoeService.saveProductInfoById(shoe.getShoeTitle(), shoe.getShoe(), shoe.getYear(),
                    shoe.getCountry(), shoe.getShoeGender(), shoe.getFragranceTopNotes(),
                    shoe.getFragranceMiddleNotes(), shoe.getFragranceBaseNotes(), shoe.getDescription(),
                    shoe.getFilename(), shoe.getPrice(), shoe.getVolume(), shoe.getType(), shoe.getId());
     ...
     * @param file    file image.
     */
    private void saveFile(Shoe shoe, @RequestParam("file") MultipartFile file) throws IOException {
        if (file == null) {
            shoe.setFilename("empty.jpg");
        } else {
            ...
            shoe.setFilename(resultFilename);
        }
    }
}
